...
export const Navbar = ({ isBirdView, onToggleBirdView }: NavbarProps) => {
  ...
  return (
    <nav className="absolute top-0 left-0 w-full p-3 sm:p-4 flex justify-between items-start pointer-events-none z-50">
      <div className="flex flex-wrap gap-2 sm:gap-4 pointer-events-auto">
        <button
          onClick={onToggleBirdView}
          className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-bold transition-all shadow-lg backdrop-blur-sm border-2 ${
            isBirdView 
